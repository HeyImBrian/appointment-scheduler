...
/**
 * This page allows the user to update the selected appointment.
 */
public class AppointmentsUpdateController {
    ...
    private ObservableList<Integer> tempID;
     ...
     * Sets all fields to the values of the appointment.
     * @throws SQLException
     */
    public void initialize() throws SQLException {
        tempAppointmentList = JDBC.getAllAppointments();
        for (int i = 0; i < tempAppointmentList.size(); i++){
            if (tempAppointmentList.get(i).getID() == inputAppointment.getID()){
                tempAppointmentList.remove(i); // Remove input appointment so that it doesn't get checked for time overlap
        ...
        fieldID.setText(String.valueOf(inputAppointment.getID()));
        fieldTitle.setText(inputAppointment.getTitle());
        fieldDesc.setText(inputAppointment.getDescription());
        fieldLoc.setText(inputAppointment.getLocation());
        datePickerDate.setValue(inputAppointment.getStart().toLocalDate());

        String tempStart = inputAppointment.getStart().format(timeFormatter);
        String tempEnd = inputAppointment.getEnd().format(timeFormatter);
        fieldStartTime.setText(tempStart);
        fieldEndTime.setText(tempEnd);

        tempCustomerObservable = JDBC.getAllCustomers();
        tempIDAndNames = FXCollections.observableArrayList();
        tempID = FXCollections.observableArrayList();
        ...
        for (int i = 0; i < tempCustomerObservable.size(); i++){
            ...
            tempID.add(Integer.valueOf(tempIDAndNames.get(i).replaceAll("\\D+","")));
        }
        comboBoxCustomer.setItems((tempIDAndNames));


        for (int i = 0; i < tempID.size(); i++){
            if (inputAppointment.getCustomerID() == tempID.get(i)){
                currSelectedIndex = i;
            }
        }
        comboBoxCustomer.getSelectionModel().select(currSelectedIndex);


        comboBoxType.setItems(FXCollections.observableArrayList(ComboBoxCollections.comboBoxAppointmentTypes));
        comboBoxType.getSelectionModel().select(inputAppointment.getType());

        comboBoxContact.getSelectionModel().select(Integer.valueOf (inputAppointment.getContactID()) - 1);

    }


    /**
     * Saves the appointment if it passes all error checks.
     * @param event
     * @throws IOException
     * @throws SQLException
     */
    public void save(ActionEvent event) throws IOException, SQLException {
        ...
        tempAppointment.setID(inputAppointment.getID());
        JDBC.updateAppointment(tempAppointment);
